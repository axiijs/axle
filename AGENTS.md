...
npx vitest run         # 仅测试（jsdom 环境，不是真实浏览器；axii 是真实浏览器，别混淆）
...
axle 是 axii 模型的移植（对照仓库在 `../axii`）。修 bug / 加语义前先看 axii 的同名实现：
行为一致的优先保持一致；**有意的差异必须在代码注释里写明**（如 `RxLeaferState.ref` 换绑先解绑旧目标、
`ElementHost` 属性 effect 先簿记后运行——axii 的 StaticHost 是先 run 后 push，存在泄漏窗口，
对齐时不要把这两处「改回去」）。
...
- Node >= 20.19；测试环境是 **jsdom + vitest-canvas-mock**（不需要 Playwright 浏览器，
  与 axii 不同）；需要真实 leafer 行为的测试（如 zIndex 物理重排、`ui.leafer`）在测试内
