...
describe('RxList triggerInfo 形态（RxListHost.applyTriggerInfo 的输入契约）', () => {
  ...
  it('set 的非整数 / 字符串 key 原样透传（data[key] = v 的属性赋值，不改变长度）', () => {
    ...
    for (const key of [1.5, NaN, undefined] as unknown as number[]) {
      list.set(key, 'x')
      const info = sub.infos[sub.infos.length - 1]!
      expect(info.type).toBe('explicit_key_change')
      expect(info.key).toBe(key)
    }
    // 规范数字字符串（'1'）:data0 ≥ 2.12 在源头把它归一化为 number 1
    //  （平台语义 data['1'] === data[1],见 data0 README「参数契约」),
    //  2.11 及以前原样透传字符串——两种形态 axle 的
    //  handleExplicitKeyChange 归一化都能正确消费,这里对两代协议都放行;
    //  升级锁定 data0 ≥ 2.12 后可收紧为只接受 number。
    list.set('1' as unknown as number, 'x')
    const stringKeyInfo = sub.infos[sub.infos.length - 1]!
    expect(stringKeyInfo.type).toBe('explicit_key_change')
    expect([1, '1']).toContain(stringKeyInfo.key)
